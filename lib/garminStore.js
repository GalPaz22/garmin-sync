...
/**
 * Describe the storefront connection a run will use, for the dashboard.
 *
 * Reports the shop URL and *whether* API credentials are present, never their
 * values — this is read by a browser, so a key or secret must never appear in
 * it. Which mode is reported matters: without WooCommerce keys the pipeline
 * silently falls back to the public `/wp-json` endpoint, which returns a
 * thinner catalog, and that is worth seeing before wondering why a run came
 * back short.
 */
export function describeConnection({ platform, credentials = {} } = {}) {
  if (platform === 'woocommerce') {
    const hasApiCredentials = Boolean(credentials.wooKey && credentials.wooSecret);
    return {
      platform,
      url: credentials.wooUrl || null,
      hasApiCredentials,
      mode: hasApiCredentials ? 'woo-rest-api' : 'woo-public-fallback'
    };
  }

  if (platform === 'shopify') {
    const hasClientCredentials = Boolean(credentials.shopifyClientId && credentials.shopifyClientSecret);
    const hasToken = Boolean(credentials.shopifyToken);
    return {
      platform,
      url: credentials.shopifyDomain || null,
      hasApiCredentials: hasClientCredentials || hasToken,
      mode: hasClientCredentials ? 'shopify-client-credentials' : hasToken ? 'shopify-access-token' : 'shopify-public-fallback'
    };
  }

  return { platform: platform || null, url: null, hasApiCredentials: false, mode: 'unknown' };
}
