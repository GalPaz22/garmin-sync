...
function createBasicProductData(product) {
  ...
  return withWooPopularityData({
    ...
    // Drafts are imported like anything else, but flagged so the consumer can
    // keep them out of results. Written on every product, not only on drafts:
    // a flag that is only ever set would stick after a product is published
    // and hide it for good.
    hidden: publicationStatus === 'draft',
    fetchedAt: new Date(),
