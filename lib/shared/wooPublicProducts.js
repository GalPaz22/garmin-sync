...
/**
 * Woo's publication status: "publish", "draft", "pending" or "private".
 *
 * The REST API carries it on the product itself. The public fallbacks normalise
 * it away into an `active` boolean but keep the raw payload under
 * `publicProductJson`, so that is read too. When neither shape carries a
 * status, null is returned rather than a guess: `active: false` cannot tell a
 * draft apart from a pending or private product.
 */
export function resolveWooProductStatus(product) {
  const direct = product?.status ?? product?.publicProductJson?.status;
  if (typeof direct === 'string' && direct.trim()) return direct.trim().toLowerCase();
  return null;
}

/**
 * Phrases that hide a product when they appear in its category breadcrumb.
 * Matched case-insensitively, anywhere in the breadcrumb text.
 */
export const HIDDEN_BREADCRUMB_TERMS = ['Neuro', 'Vibey'];

/** Collect breadcrumb text out of whichever shape the payload uses. */
function collectBreadcrumbText(source) {
  if (!source || typeof source !== 'object') return [];

  const parts = [];

  // An explicit breadcrumb, when the storefront sends one: a string, or a
  // trail of either strings or { name }/{ title } crumbs.
  for (const field of [source.breadcrumb, source.breadcrumbs]) {
    if (typeof field === 'string') parts.push(field);
    else if (Array.isArray(field)) {
      for (const crumb of field) {
        if (typeof crumb === 'string') parts.push(crumb);
        else if (crumb && typeof crumb === 'object') parts.push(crumb.name || crumb.title || '');
      }
    }
  }

  // Otherwise the categories the product sits in are the breadcrumb: the slug
  // goes in too, because a Hebrew category name often has a latin slug and the
  // phrases being matched are latin.
  if (Array.isArray(source.categories)) {
    for (const category of source.categories) {
      if (typeof category === 'string') parts.push(category);
      else if (category && typeof category === 'object') {
        parts.push(category.name || category.title || '', category.slug || '');
      }
    }
  }
...
/**
 * The product's category breadcrumb as one searchable string.
 *
 * Read from the product and from the raw payload the public fallbacks keep
 * under publicProductJson, so a breadcrumb survives normalisation.
 */
export function resolveWooBreadcrumb(product) {
  return [
    ...collectBreadcrumbText(product),
    ...collectBreadcrumbText(product?.publicProductJson)
  ].filter(Boolean).join(' / ');
}

/**
 * The phrase that hides this product, or null when none does.
 *
 * Returns the phrase rather than a boolean so the document can record why the
 * product was hidden instead of only that it was.
 */
export function matchHiddenBreadcrumbTerm(product, terms = HIDDEN_BREADCRUMB_TERMS) {
  const breadcrumb = resolveWooBreadcrumb(product).toLowerCase();
  if (!breadcrumb) return null;
  return terms.find(term => term && breadcrumb.includes(String(term).toLowerCase())) || null;
}

/**
 * Draft products are still imported — they simply carry `hidden: true` so the
 * consumer can keep them out of results. Anything whose status is unknown is
 * never flagged: hiding a product on a guess is worse than showing a draft.
 */
export function isWooDraft(product) {
  return resolveWooProductStatus(product) === 'draft';
}
